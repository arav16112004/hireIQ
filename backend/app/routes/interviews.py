...
from ..core.deps import get_gemini_service, get_elevenlabs_service, get_did_service, get_webgazer_service
...
def start_interview(candidate_id: int, job_id: int, gemini=Depends(get_gemini_service)):
    job = snowflake_client.get_job_by_id(job_id)
    if not job:
...
@router.post("/v1/interviews/next")
def next_interview(
    session_id: str,
    response_text: str,
